...
void criarFila( struct Fila *f, int c) { 
	...
	int tamanho = 50;
	f->capacidade = c;
//  f->nome = (char*) malloc (tamanho * sizeof(char));
	f->dados = (float*) malloc (f->capacidade * sizeof(float));
//	f->telefone = (long int*) malloc (f->capacidade *sizeof(long int));
//	f->cpf = (long int*) malloc (f->capacidade *sizeof(long int));
//	f->telefone = (long int*) malloc (f->capacidade *sizeof(long int));
//	f->endereco = (char*) malloc (tamanho * sizeof(char));
	f->primeiro = 0;
	f->ultimo = -1;
	f->nItens = 0; 
...
void inserir(struct Fila *f, int i) {

	if(f->ultimo == f->capacidade-1)
		f->ultimo = -1;

	f->ultimo++;
	f->dados[f->ultimo] = i; // incrementa ultimo e insere
...
int main (int argc, char** argv) {
	...
	while( 1 ){
		...
		printf("\n1 - Inserir visita na fila \n2 - Visitar ocorr�ncia(apaga da fila)\n 3 - Mostrar visitas a serenm feitas\n0 - Sair \n \n O que deseja fazer? ");
		scanf("%d", &opcao);
